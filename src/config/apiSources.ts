...
/**
 * Path to the root API sources file
 */
const API_SOURCES_FILE = path.resolve(process.cwd(), 'api - Copy.txt');
...
function readAPISourcesFile(): APISourcesCache {
  ...
  try {
    if (!fs.existsSync(API_SOURCES_FILE)) {
      cache = {};
      lastReadTime = now;
      return cache;
    }

    const content = fs.readFileSync(API_SOURCES_FILE, 'utf-8');
